...
## Supervised Attention Module ，不用看没有用到过----------------------------------------------------------------------------
class SAM(nn.Module):
...
#########################################这一块也没啥用，是Unet模型的原始版本-----------------------。之所以这里要写上是因为作者在原文图一中进行了计算复杂度的对比-------------------------
...
# 从这开始，下面的才是本文中模型得代码实现，当然，其中有一部分代码是原始模型的代码，本模型的代码是基于原始模型代码改进过来的。！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
#下面的LEFF模块有用到这个方法，本文用的是eca_layer_1d，但事实上用eca_layer也是可以的，实现上并无差别，这是ECANet代码里面的eca——module模块----------------------------------------------------------------------------------------------------
...
########################################这是一个深度可分离卷积实现，将标准的卷积分为两个步骤：深度卷积(depthwise)和逐点卷积(pointwise)，这种结构的优势在于减少参数量，##############################
...
############################################# Embedding for q,k,v。一个是利用卷积进行一次特征提取改变维度，一个是直接进行线性投影 ####################################
...
################################################ window-based self-attention是在下面的self-attention模块的基础上改过来的，下面的self-attention相当于是一个参考#################################################################
class WindowAttention(nn.Module):
    def __init__(self, dim, win_size,num_heads, token_projection='linear', qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # define a parameter table of relative position bias，这里用的是相对位置编码，参考Vision Transformer
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * win_size[0] - 1) * (2 * win_size[1] - 1), num_heads))  # 2*Wh-1 * 2*Ww-1, nH

        # get pair-wise relative position index for each token inside the window，构建并初始化相对位置偏置参数表格
        ...
        elif token_projection =='linear':    #默认选Linear的投影方式
    ...
    def forward(self, x, attn_kv=None, mask=None):
        ...
        q = q * self.scale
        attn = (q @ k.transpose(-2, -1))   #计算注意力分数矩阵
        ...
        attn = attn + relative_position_bias.unsqueeze(0)   #在自注意力中引入相对位置偏移
...
######################################################################## self-attention，在cross_modulator为True的时候才会用到 ###########################################
...
#--------------------------------------------------------------------------------------------transformer里面原始的mlp模块和改进的Leff模块---------------------------------------------------------------------------------
...
###############################，将划分的张量进行拼接成为原来的原始张量形状，与上面的是一对的#################################################
...
# -------------------------------------------------------Input Projection,对应论文中的输入和输出投影------------------------------------------------------------------------------------------
class InputProj(nn.Module):
    def __init__(self, in_channel=3, out_channel=64, kernel_size=3, stride=1, norm_layer=None,act_layer=nn.LeakyReLU):
        super().__init__()   #继承了模块就是必要的
    ...
    def forward(self, x):
        B, C, H, W = x.shape   #B是批量大小
        #原本是(B,C,H,W)经过flatten(2)后变为(B,C,H*W)
        x = self.proj(x).flatten(2).transpose(1, 2).contiguous()  # B H*W C
        if self.norm is not None:
            x = self.norm(x)
        return x
        #return x.view(tensor.size(0),tensor.size(1),-1)

    def flops(self, H, W):   #这是他写的复杂度计算的方法
        flops = 0
        # conv
...
class LeWinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8, shift_size=0,
        ...
        if modulator:  #对应论文的Multi-Scale Restoration Modultor
            self.modulator = nn.Embedding(win_size*win_size, dim) # modulator
        else:
            self.modulator = None

        if cross_modulator:  #这里的cross_modulator原文似乎没有提到
        ...
        self.attn = WindowAttention(
            ...
            token_projection=token_projection)    #Window based self attention获得对应的权重矩阵
    ...
    def forward(self, x, mask=None):
        ...
        if self.cross_modulator is not None:     #调制器信息的加入
            shortcut = x
        ...
        shortcut = x
        x = self.norm1(x)
        x = x.view(B, H, W, C)   #view用来改变张量的形状
        ...
        x_windows = x_windows.view(-1, self.win_size * self.win_size, C)  # nW*B, win_size*win_size, C   #nw*b的意思是一共有样本*窗口个，win_size*win_size是窗口内的像素点，C是每一个像素用C长度的tensor表示
...
#--------------------------------------------------------------------------------调用LeWinTransformerBlock初始化每一个Lewin Blocks里面的Block--------------------------------------------------------------
...
#-------------------------------------------------------------------不同的Uformer模型会有不同的不同的参数定义，整体的信息--------------------------------------------------------------------------------------------
class Uformer(nn.Module):
    #UfomerB的时候 depths是[1, 2, 8, 8, 2, 8, 8, 2, 1]
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
        ...
        self.num_enc_layers = len(depths)//2    #4个encoder和decoder
        ...
        self.dd_in = dd_in    #输入的是RGB

        # stochastic depth  定义encoder和decoder等在每一个阶段的dropout率
        #torch.linspace(0, drop_path_rate, sum(depths[:self.num_enc_layers]))是一个生成num_enc_layers维0
        # 到drop_path_rate的tensor，每个数值之间是等间隔的
        #x.item()可以把每一个tensor元素转化为标量
        ...
        self.decoderlayer_0 = BasicUformerLayer(dim=embed_dim*16,
                            ...
                            use_checkpoint=use_checkpoint,
                            token_projection=token_projection,token_mlp=token_mlp,shift_flag=shift_flag,
                            modulator=modulator,cross_modulator=cross_modulator)   #与上面的encoder和bottleneck相比多了这一行
