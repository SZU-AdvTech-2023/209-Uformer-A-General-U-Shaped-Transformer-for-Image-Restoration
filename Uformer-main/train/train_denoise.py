...
######### parser ###########
'''
下面这个opt首先创建了一个argparse.ArgumengParser对象，desciption是用户help时会返回的参数，(2)然后调用通过调用 options.Options() 
类的 init 方法，将 argparse.ArgumentParser 对象传递给它。这个方法会初始化一些选项，例如默认值、类型检查等
（3）调用 parse_args() 方法来解析命令行参数。这个方法会返回一个包含解析后的参数值的对象
'''
...
os.environ["CUDA_VISIBLE_DEVICES"] = "0"#opt.gpu  这里我改了
import torch
...
######### Logs dir ###########
#./logs/denoising/sidd/Uformer_B_0706
